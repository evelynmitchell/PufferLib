...
This is PufferLib a Reinforcement Learning Library written in C and Python with some bash scripts..
...
**Integration Tests - The Nuanced Approach to Mocking**:

Integration tests verify that components work together. The key is distinguishing **internal components** (part of the integration) from **external dependencies** (I/O, side effects, expensive operations).

**Use REAL objects for**:
- Internal components being integrated (the actual modules/classes you're testing together)
- Cheap, standard dependencies that are part of the integration contract
  - Example: `gym.spaces.Discrete(4)` - fast, part of RL API contract
  - Example: `torch.tensor()` - if testing PyTorch integration
  - Example: Dataclasses and configuration objects
- Objects whose behavior is critical to the integration

**Use MOCKS for**:
- External systems (databases, networks, APIs, file systems)
- Expensive operations (actual game simulation, GPU training, large model inference)
- Non-deterministic behavior (timestamps, random external data)
- Side effects you want to verify without executing (logging, writes, API calls)

**Example - Testing cleanrl_ppo_atari**:
```python
import pytest
import gymnasium as gym  # Real gym - part of RL integration
from unittest.mock import Mock, patch
from pufferlib.cleanrl_ppo_atari import Args, Agent

def test_agent_with_discrete_action_space():
    """Test Agent works with real gym Discrete action space."""
    ...
    assert agent.actor.out_features == 4  # Verify integration worked

@patch('pufferlib.cleanrl_ppo_atari.gym.make')  # Mock expensive environment creation
def test_make_env_without_actual_atari(mock_gym_make):
    """Test make_env creates environment thunk without loading Atari ROMs."""
    ...
    # Verify we can create the thunk without actually loading the ROM
```

**The principle**: Mock at the boundaries (I/O, expensive ops), use real objects for the integration logic itself.
